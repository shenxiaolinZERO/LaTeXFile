...
\begin{align}\label{equ:skyline}
        ...
        =p(d|\Theta^t,g)=\Pi_{w \in W^d, v\in L^d} \textcolor{red}{a^K} \frac{w_k}{w_k+\theta^t v_k}\Pi_{k'\neq k}\frac{\theta^t w_{k'}}{v_{k'}+\theta^t w_{k'}}
\end{align}
Besides,
\begin{eqnarray}
...
\min -\sum_{R,w \succ_R v} a^K \Sigma_{u(d)=u} \Sigma_{k=1}^K \gamma(d,k,\Theta^t) \ln u_k\\ \nonumber
w.r.t \Sigma_k u_k =1
\end{align}
...
u_k =\frac{\sum_{R,w \succ_R v} a^K \Sigma_{u(d)=u}\gamma(d,k,\Theta^t)}{ \sum_{R,w \succ_R v} a^K \Sigma_{s=1}^K \Sigma_{u(d)=u}\gamma(d,s,\Theta^t)}
\end{equation}
